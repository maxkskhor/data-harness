...
class TestMaxTurnReminder:
    def test_reminder_in_last_user_message_before_final_call(self, tmp_path):
        """Before the final allowed call (turn max_turns - 1), reminder is in user message."""
        # max_turns=3: turns 1, 2, 3. At turn 2 (max_turns - 1), reminder is added.
        tool = ToolSpec(name="noop", description="noop", input_schema={}, handler=lambda: "ok")
        adapter = FakeAdapter([
            make_tool_response("tu_1", "noop", {}),  # turn 1
            make_tool_response("tu_2", "noop", {}),  # turn 2 (last before max)
            make_text_response("done"),               # turn 3 (max)
        ])
        harness = Harness(adapter=adapter, system="sys", tools=[tool], max_turns=3, run_dir=str(tmp_path))
        harness.run("go")
        # Turn 2 (index 1) call messages: should include reminder
        turn2_messages = adapter._calls[1]["messages"]
        all_text = " ".join(
            b.text for m in turn2_messages for b in m.content if isinstance(b, TextBlock)
        )
        assert any(word in all_text.lower() for word in ["final", "last", "output", "respond", "turn"])

    def test_reminder_appended_to_tool_result_user_message(self, tmp_path):
        """If current user message is a tool_result message, reminder is appended as TextBlock there."""
        tool = ToolSpec(name="t", description="t", input_schema={}, handler=lambda: "ok")
        adapter = FakeAdapter([
            make_tool_response("tu_1", "t", {}),  # turn 1
            make_text_response("done"),            # turn 2 (max_turns=2 → turn 1 = max_turns-1)
        ])
        # With max_turns=2, turn 1 is max_turns - 1 = 1
        harness = Harness(adapter=adapter, system="sys", tools=[tool], max_turns=2, run_dir=str(tmp_path))
        harness.run("go")
        ...
        has_tool_result = any(isinstance(b, ToolResultBlock) for b in last_user.content)
        has_text = any(isinstance(b, TextBlock) for b in last_user.content)
        # At max_turns-1 = turn 1, the reminder should be in the first call's user message
        turn1_messages = adapter._calls[0]["messages"]
        last_user_t1 = [m for m in turn1_messages if m.role == "user"][-1]
        all_text_t1 = " ".join(b.text for b in last_user_t1.content if isinstance(b, TextBlock))
        assert any(word in all_text_t1.lower() for word in ["final", "last", "output", "respond", "turn"])
    ...
    def test_system_prompt_byte_stable_with_reminder(self, tmp_path):
        ...
        harness = Harness(adapter=adapter, system=system, tools=[tool], max_turns=2, run_dir=str(tmp_path))
        harness.run("go")
    ...
    def test_new_user_message_created_when_no_current_user(self, tmp_path):
        ...
        turn1_messages = adapter._calls[0]["messages"]
        all_text = " ".join(b.text for m in turn1_messages for b in m.content if isinstance(b, TextBlock))
